...
/* UltimateButtonEditor.cs ver 1.5.2 */
using UnityEngine;
using UnityEditor;
...
public class UltimateButtonEditor : Editor
{
	...
	void OnEnable ()
	{
		...
		if( myParent != null && !myParent.GetComponent<UltimateButtonUpdater>() )
			myParent.AddComponent( typeof( UltimateButtonUpdater ) );
	}
	...
	*/
	public override void OnInspectorGUI ()
	{
		...
		if( EditorGUILayout.BeginFadeGroup( TouchActions.faded ) )
		{
			...
			{
				serializedObject.ApplyModifiedProperties();
				tapOption.target = ult.tapCountOption != UltimateButton.TapCountOption.NoCount ? true : false;
			}
			if( EditorGUILayout.BeginFadeGroup( tapOption.faded ) )
			{
				EditorGUI.indentLevel = 1;
				{
					EditorGUI.BeginChangeCheck();
					{
						...
						if( ult.tapCountOption == UltimateButton.TapCountOption.Accumulate )
		...
		if( EditorGUILayout.BeginFadeGroup( ScriptReference.faded ) )
		{
			...
			if( ult.referenceOption == UltimateButton.ReferenceOption.GetButtonStates )
			{
				EditorGUILayout.Space();
				EditorGUI.BeginChangeCheck();
				...
				else
				{
					...
					EditorGUI.indentLevel = 1;
					EditorGUILayout.TextField( "UltimateButton.GetButtonDown( \"" + ult.buttonName + "\" )" );
					EditorGUI.indentLevel = 0;

					EditorGUILayout.LabelField( "Get Button:" );
					EditorGUI.indentLevel = 1;
					EditorGUILayout.TextField( "UltimateButton.GetButton( \"" + ult.buttonName + "\" )" );
					EditorGUI.indentLevel = 0;

					EditorGUILayout.LabelField( "Get Button Up:" );
					EditorGUI.indentLevel = 1;
					EditorGUILayout.TextField( "UltimateButton.GetButtonUp( \"" + ult.buttonName + "\" )" );
					EditorGUI.indentLevel = 0;
				}
			}
			else
